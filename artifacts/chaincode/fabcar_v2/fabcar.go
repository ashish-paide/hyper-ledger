...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	case "addAssestsInPrivateData":
		return s.addAssestsInPrivateData(APIstub , args)
	default:
...
func(s * SmartContract)addAssestsInPrivateData(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	type AssetTransiantInput  struct {
		Id             string `json:"id"`
		Owner          string `json:"owner"`
	}
	if len(args) != 0 {
	...
	var assetInput AssetTransiantInput
	err = json.Unmarshal(Assets, &assetInput)
	if err != nil {
		return shim.Error("44444 -Failed to decode JSON of: " + string(Assets) + "Error is : " + err.Error())
	}

	logger.Infof("3333")

	if len(assetInput.Id) == 0 {
		return shim.Error("ID should be non empty")
	}
	if len(assetInput.Owner) == 0 {
		return shim.Error("Owner should be non empty")
	}
	

	logger.Infof("444444")
	Addasset := AssetTransiantInput{Id: assetInput.Id, Owner: assetInput.Owner}
	assetAsBytes , err := json.Marshal(Addasset)
	if(err != nil){
		return shim.Error(err.Error())
	}
	logger.Infof("555555")
	err = APIstub.PutPrivateData("collectionAssets" , Addasset.Id  , assetAsBytes)
	if(err != nil){
		logger.Infof("666666")
		return shim.Error(err.Error())
	}
...
func (s *SmartContract) restictedMethod(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	// get the MSP ID of the client's identityinput"
